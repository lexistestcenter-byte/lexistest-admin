...
function QuestionDetailPreview({ question }: { question: Question }) {
  const od = question.options_data || {};
  const ad = question.answer_data || {};
  const fmt = question.question_format;

  // Extract typed data outside JSX to avoid 'unknown' in ReactNode issues
  const mcqOptions: { label: string; text: string; isCorrect?: boolean }[] =
    Array.isArray(od.options) ? (od.options as { label: string; text: string; isCorrect?: boolean }[]) : [];

  const tfngItems: { number: number; statement: string }[] =
    Array.isArray(od.items) ? (od.items as { number: number; statement: string }[]) : [];

  const matchOptions: { label: string; text: string }[] =
    Array.isArray(od.options) ? (od.options as { label: string; text: string }[]) : [];

  const matchItems: { number: number; statement: string }[] =
    Array.isArray(od.items) ? (od.items as { number: number; statement: string }[]) : [];

  const wordBank: string[] =
    Array.isArray(od.word_bank) ? (od.word_bank as string[]) :
    Array.isArray(od.wordBank) ? (od.wordBank as string[]) : [];

  const blanks: { number: number; answer: string }[] =
    Array.isArray((ad as Record<string, unknown>).blanks)
      ? ((ad as Record<string, unknown>).blanks as { number: number; answer: string }[])
      : [];

  const tfngAnswer = String((ad as Record<string, unknown>)?.answer ?? "—");
  ...
  return (
    <div className="mt-2 p-3 bg-white border rounded-lg text-sm space-y-2">
      ...
      {/* MCQ */}
      {(fmt === "mcq_single" || fmt === "mcq_multiple") && mcqOptions.length > 0 && (
      ...
      {fmt === "true_false_ng" && (
        <div className="space-y-1">
          {tfngItems.length > 0 ? (
            ...
            <div className="text-xs text-gray-500">답: {tfngAnswer}</div>
          )}
        </div>
      )}
      ...
      {/* 빈칸 - word bank */}
      {(fmt === "fill_blank_drag" || fmt === "table_completion") && wordBank.length > 0 && (
      ...
      {/* 정답 (빈칸 계열) */}
      {(fmt === "fill_blank_typing" || fmt === "fill_blank_drag" || fmt === "flowchart" || fmt === "table_completion") &&
        blanks.length > 0 && (
          <div className="text-xs space-y-0.5">
